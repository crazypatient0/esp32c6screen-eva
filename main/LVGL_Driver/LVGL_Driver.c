...
void example_lvgl_flush_cb(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    // adjust offset for landscape rotation: offset moves from X to Y
    int off_x = (drv->rotated == LV_DISP_ROT_NONE || drv->rotated == LV_DISP_ROT_180) ? Offset_X : 0;
    int off_y = (drv->rotated == LV_DISP_ROT_NONE || drv->rotated == LV_DISP_ROT_180) ? 0 : Offset_X;
    esp_lcd_panel_draw_bitmap(panel_handle, offsetx1 + off_x, offsety1 + off_y, offsetx2 + off_x + 1, offsety2 + off_y + 1, color_map);
}
